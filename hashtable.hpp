 ...
 * >> map должна быть шаблонной по ключу и значению. По желанию - возможность
 * принять HashT, ComparatorT.
...
class HashTable {
  struct Cell {
    ...
    Cell(const K &key, const T &item, const size_t &hash)
        : _key(key), _item(item), _hash(hash), _state(CELL_KEY) {}
  };

  size_t _size;

  /* Коэффициент заполнения хеш-таблицы (load factor) вычисляется как отношение
   * числа хранимых элементов к размеру массива (числу возможных значений
   * хеш-функции). Обозначается как alpha = N/M, где N — количество элементов в
   * хеш-таблице, M — размер хеш-таблицы.
   * - Этот коэффициент является важным параметром, от которого зависит среднее
   * время выполнения операций.
   * - Значение коэффициента говорит о степени заполненности таблицы: 2
   * Пока коэффициент меньше единицы, в массиве есть свободные элементы, куда
   * теоретически ещё можно добавить новые ключи.
...
public:
  //// Member functions
  HashTable(const size_t &sizeTable = 4, const float &factor = 1.0)
      : _size(0), _factor(factor), _table(sizeTable), _count(0) {}

  HashTable(const HashTable<K, T, Hash> &rhs)
      : _size(rhs._size), _factor(rhs._factor), _table(rhs._table) {}

  HashTable(HashTable<K, T, Hash> &&rhs)
      : _size(rhs._size), _factor(rhs._factor), _table(rhs._table) {}
  ...
  const_iterator cbegin() noexcept {
    return const_iterator(_table.cbegin(), _table.cend());
  }
  ...
  const_iterator cend() noexcept {
    return const_iterator(_table.cend(), _table.cend());
  }
  ...
  std::pair<iterator, bool> insert(const K &key, const T &item) {
    ...
    return std::pair<iterator, bool>(
        iterator((_table.begin() + hash), _table.end()), true);
  }
  ...
  /** @brief erase - Удаление по ключу, возвращает 1 если элемент был удален, 0
  ...
  /** @brief reserve - в оригинальном STL если значение меньше _table.size() ->
