...
private:
  ...
  bool checkWeight(const size_t &bXleft, const size_t &bXright,
                   const Bag &bag) {
  ...
  fillAdjacentPairs(const size_t &neededFreeSpace) {
...
public:
  Segment() {}
  Segment(const size_t &max_Wieght)
      : currentWeight(0), maxWieght(max_Wieght), bags_group((maxWieght + 1)) {}
  ~Segment() {
...
private:
  ...
  std::array<Bag, 4> unpackBagsFromLine(const size_t &id,
  ...
  bool toRegister(std::ostream &out, const std::string &type, const size_t &id,
    ...
    if (((bool_p.first and bool_p.second) or
         (bool_p.first and !bool_p.second and passenger->name == "ECONOMY"))) {
  ...
  template <typename T>
  void put(std::ostream &out, const std::string &type,
           const std::string &true_type, const std::array<Bag, 4> &bags,
           const std::string &cs) {
  ...
  void passengerUpBoard(std::ostream &out,
                        const std::vector<std::string> &passenger) {
