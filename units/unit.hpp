...
struct Bag {
  ...
  Bag(const Bag &other)
      : weight(other.weight), passengerId(other.passengerId) {}
...
public:
  ...
  Passenger(const size_t &id, const std::array<Bag, 4> &bags,
            const size_t &handBag, const size_t &handBagValue,
            const size_t &bag, const size_t &bagValue,
            const std::string &_name = "")
      : _id(id), _bags(bags), _handBag(handBag), _handBagValue(handBagValue),
        _bag(bag), _bagValue(bagValue), name(_name) {}
  ~Passenger() {}
